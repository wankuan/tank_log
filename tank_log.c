...
log_status_t get_current_time_str(uint8_t *p_timer)
{
    struct timeval tv;
    struct tm  *p;
    if(p_timer==NULL_PTR)
    ...
    snprintf((char*)p_timer, 32, "%04d-%02d-%02d %02d:%02d:%02d.%03d", 1900+p->tm_year, 1+p->tm_mon, p->tm_mday, p->tm_hour, p->tm_min, p->tm_sec, tv.tv_usec/1000);
    return LOG_SUCCESS;
}
